...
RSpec.describe API::Controllers::Audit::Quote do
  include Support::HTTPStubbing

  let(:params) {
    { property_id: "success", check_in: "2016-03-22", check_out: "2016-03-25", guests: 2 }
  }
  ...
  it_behaves_like "external error reporting" do
    let(:params) {
      { property_id: "success", unit_id: "123", check_in: "2016-03-22", check_out: "2016-03-25", guests: 2 }
    }
    let(:supplier_name) { "Audit" }
    let(:error_code) { "savon_erorr" }

    def provoke_failure!
      credentials = Concierge::Credentials.for("audit")
      stub_call(:get, URI.join(credentials.host, "/spec/fixtures/audit/quotation.success.json").to_s) { raise Faraday::Error.new("oops123") }
      Struct.new(:code).new("network_failure")
    end
  end
  ...
  describe "#call" do
    ...
    it "returns a proper error message if client returns quotation with error" do
      expect_any_instance_of(Audit::Client).to receive(:quote).and_return(Result.error(:network_error))
    ...
    it "returns unavailable quotation when client returns so" do
      ...
      expect_any_instance_of(Audit::Client).to receive(:quote).and_return(Result.new(unavailable_quotation))

      response = parse_response(subject)
      expect(response.status).to eq 200
      expect(response.body["status"]).to eq "ok"
      expect(response.body["available"]).to eq false
      expect(response.body["property_id"]).to eq "success"
    ...
    it "returns available quotation when call is successful" do
      ...
      expect_any_instance_of(Audit::Client).to receive(:quote).and_return(Result.new(available_quotation))
